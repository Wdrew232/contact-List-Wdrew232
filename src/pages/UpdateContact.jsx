...
export const UpdateContact = () => {
  ...
  useEffect(() => {
    // Ensure local state updates if contact details change
    if (contact) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        dispatch({
          ...
          payload: store.contactList.map((item) =>
            item.id === parseInt(id) ? updatedContact : item
          ),
        });
        navigate("/contacts"); // Redirect to contact list after update
      } else {
